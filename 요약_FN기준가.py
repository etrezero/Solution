...
# 파일 읽어오기
try:
    df = pd.read_excel(path_요약, sheet_name='FN기준가')
...
T_7 = today_date - timedelta(days=10)
...
if workbook is not None:
        ...
        path_FN기준가 = os.path.join('C:\Covenant\data\To_요약', file_FN기준가)

        df.to_excel(path_FN기준가, sheet_name='FN기준가', index=False)


        # Workbook_FN기준가 열기
        workbook_FN기준가 = excel.Workbooks.Open(path_FN기준가)
        time.sleep(0.1)
