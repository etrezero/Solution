...
def fetch_ticker_data(url, exchange_name):
    """
    Fetch ticker data from NASDAQTrader or NYSE FTP.
    """
    try:
        ...
        # 데이터프레임 생성
        df = pd.DataFrame([line.split("|") for line in lines], columns=lines[0].split("|"))
        df = df[['Symbol', 'Security Name']]  # 필요한 열 선택
        df.columns = ['Ticker', 'Company Name']  # 열 이름 변경
        print(f"Fetched {len(df)} tickers from {exchange_name}.")
        return df
    except Exception as e:
        print(f"Failed to fetch {exchange_name} data: {e}")
        return pd.DataFrame()


# NASDAQ 및 NYSE 데이터 다운로드
nasdaq_url = "https://www.nasdaqtrader.com/dynamic/SymDir/nasdaqlisted.txt"
nyse_url = "https://www.nasdaqtrader.com/dynamic/SymDir/otherlisted.txt"

nasdaq_tickers = fetch_ticker_data(nasdaq_url, "NASDAQ")
nyse_tickers = fetch_ticker_data(nyse_url, "NYSE")

# 데이터 병합 및 저장
combined_tickers = pd.concat([nasdaq_tickers, nyse_tickers], ignore_index=True)

US_list = r'C:\Covenant\data\US_list.csv'
if not combined_tickers.empty:
    combined_tickers.to_csv(US_list, index=False)
    print(f"Saved combined ticker data to {US_list}.")
else:
    print("Combined ticker data is empty. Check the source files or URLs.")
