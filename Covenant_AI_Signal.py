...
# LSTM 데이터를 생성하는 함수
def create_lstm_data(df_price, target_col, time_steps=20):
    scaler = MinMaxScaler()
...
def create_lstm_model(input_shape):
    model = Sequential()
    model.add(LSTM(20, return_sequences=True, input_shape=(input_shape[1], 1)))
    model.add(LSTM(20))
    model.add(Dense(1))
...
def update_graph(stock_code, selected_year):
    ...
    # LSTM 시그널 생성
    X, y, scaler = create_lstm_data(df_price, stock_code, time_steps=20)
