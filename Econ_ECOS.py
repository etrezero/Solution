...
# 이 옵션은 “초기 layout에는 없지만 나중에 생길 수도 있는 컴포넌트 ID를 콜백에서 참조하겠다”는 것을 Dash에게 알리는 역할
...
@app.callback(
    ...
    State('term', 'value'),  # ✅ term 값을 위한 드롭다운 추가
...
def filter_and_plot(n, val1, val2, val3, val4, term):
    ...
    try:
        if term == 'A':  # 연도
            ...
            df = df.dropna(subset=['TIME'])  # ❗ 오류난 날짜 제거
        elif term == 'D':  # 일간
            df['TIME'] = pd.to_datetime(df['TIME'], format='%Y%m%d')
        else:
            raise ValueError(f"지원하지 않는 term 값: {term}")
