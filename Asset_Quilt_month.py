...
def save_excel(df, sheetname, index_option=None):
    ...
    path = rf'C:\Covenant\data\Asset_Quilt_월간.xlsx'
...
# 캐싱 경로 및 만료 시간 설정
cache_price = r'C:\Covenant\cache\자산군별수익률_월간.pkl'
...
start = (datetime.today() - relativedelta(years=3) - timedelta(days=1) ).strftime('%Y-%m-%d')
...
월간수익률 = ETF_price.resample('ME').last()

# save_excel(월간수익률, '월간수익률', index_option=False)


월간수익률 = 월간수익률.pct_change()
월간수익률.index = 월간수익률.index.strftime('%Y-%m')  #그룹바이를 안하면 연도 형식지정



# '자산배분'을 별도로 처리하려면, 월간수익률.columns의 값이 '자산배분'인 경우를 예외 처리해주면 됩니다.
# 월간수익률의 열 이름을 code_dict의 키로 설정
월간수익률.columns = [
    list(code_dict.keys())[list(code_dict.values()).index(col)] if col in code_dict.values() else col
    for col in 월간수익률.columns
]


# 딕셔너리 코드 값에 포함된 자산군만 필터링 (자산배분 제외) - 중복 제거
valid_columns = list(dict.fromkeys([col for col in 월간수익률.columns if col in code_dict])) + ['자산배분']

# 유효한 자산군만 포함한 월간수익률
월간수익률 = 월간수익률[valid_columns]
...
for col in 월간수익률.columns:
    # 열을 내림차순 정렬
    sorted_column = 월간수익률[[col]].sort_values(by=col, ascending=False)
...
columns = [{'name': f'{col}', 'id': col} for col in 월간수익률.columns]
...
ETF_R_W = ETF_R_W.dropna(how='all', axis=1).fillna(0)
...
변동성 = ETF_R_W.rolling(window=4).std()*np.sqrt(52)
변동성 = 변동성.dropna()
변동성.index = pd.to_datetime(변동성.index, errors='coerce')  # 날짜 형식으로 변환
변동성 = 변동성.resample('ME').last()  # 월별 데이터 추출

변동성.index = 변동성.index.strftime('%Y-%m')  # 벡터화된 방식으로 날짜 포맷 변경
...
# 샤프 지수 계산=======================================================
# # 월간수익률의 열을 문자열로 변환
# 월간수익률.columns = 월간수익률.columns.astype(str)

# # 변동성의 열을 문자열로 변환
변동성.columns = 변동성.columns.astype(str)

# 공통 열 찾기
common_columns = 월간수익률.columns.intersection(변동성.columns)
...
if common_columns.empty:
    ...
    # 공통 열로 데이터프레임을 merge
    merged_df = pd.merge(월간수익률[common_columns], 변동성[common_columns], 
...
for col in df_수익률.columns[-12:] :  # 첫 번째 컬럼은 자산군이므로 제외
...
app.layout = html.Div(
    ...
    children=[
        html.H3(f"월간 자산군별 수익률           as of {end}", style={'textAlign': 'center'}),
        
        dash_table.DataTable(
            id='asset_return_table',
            data=수익률_table_data,  # 테이블 데이터를 DataTable에 전달
            columns=columns[-12:], # 첫번째 컬럼은 Nan이므로 제외
...
# 변동성 테이블 추가
        html.H3("월간 자산군 변동성", style={'textAlign': 'center'}),  # 제목 추가
        dash_table.DataTable(
            id='annual_volatility_table',
            data=변동성_table_data,  # 연간 변동성 데이터
            columns=columns[-12:],  # 연간 변동성 테이벌 첫째열 제거
            style_table={'height': 'auto', 'overflowY': 'auto'},  # 테이블 높이 설정
            style_cell={
                'textAlign': 'center', 
                'verticalAlign': 'middle',  # 텍스트를 세로로 중앙 정렬
...
# 위험대비수익률 테이블 추가
        html.H3("월간 위험대비수익률", style={'textAlign': 'center'}),  # 제목 추가
        dash_table.DataTable(
            id='sharpe_table',
            data=샤프_table_data,  
            columns=sharpe_columns[-12:],  
            style_table={'height': 'auto', 'overflowY': 'auto'},  # 테이블 높이 설정
            style_cell={
                'textAlign': 'center', 
                'verticalAlign': 'middle',  # 텍스트를 세로로 중앙 정렬
