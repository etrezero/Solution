...
# 파일 읽어오기
try:
    df = pd.read_excel(save_path, sheet_name='FN설정액')
...
if workbook is not None:
        ...
        file_FN설정액 = f'FN설정액.xlsx'          #file_FN설정액 = f'FN설정액 {today}.xlsx'
        path_FN설정액 = os.path.join('C:\Covenant\data\To_요약', file_FN설정액)

        df.to_excel(path_FN설정액, sheet_name='FN설정액', index=False)
        
        

        # Workbook_FN설정액 열기
        workbook_FN설정액 = excel.Workbooks.Open(path_FN설정액)
        time.sleep(0.1)
        worksheet_FN설정액 = workbook_FN설정액.Sheets('FN설정액')
        ...
        worksheet_요약_FN설정액 = workbook_요약.Sheets('FN설정액')
        worksheet_요약_FN설정액.Activate()
        time.sleep(0.1)



        # 현재 사용 중인 행의 개수 파악
        max_row_요약 = worksheet_요약_FN설정액.UsedRange.Rows.Count
        print(f"최대행은 {max_row_요약}행 입니다")

        # A열의 최대 행의 값 가져오기
        max_value_요약 = worksheet_요약_FN설정액.Cells(max_row_요약, 1).Value

        # 복사할 행 선택
        for row_num in range(14, worksheet_FN설정액.UsedRange.Rows.Count + 1):
            row_data = []  # 각 행마다 초기화

            cell_value = worksheet_FN설정액.Cells(row_num, 1).Value

            # 현재 행의 A열 값이 최대 행의 A열 값보다 큰 경우 데이터를 복사
            if isinstance(cell_value, str) or str(cell_value) > str(max_value_요약):
                # 데이터를 복사
                row_data = [worksheet_FN설정액.Cells(row_num, col_num).Value for col_num in range(1, worksheet_FN설정액.UsedRange.Columns.Count + 1)]
                ...
                for col_num, value in enumerate(row_data, start=1):
                    worksheet_요약_FN설정액.Cells(next_row, col_num).Value = value
                
                print(f"행 {row_num}을 복사하여 요약_FN설정액 {next_row}행에 붙여넣었습니다.")
                max_row_요약 += 1
